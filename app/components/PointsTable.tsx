...
export default function PointsTable({ rows }: Props) {
  return (
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      <div className="px-4 py-2 text-sm font-semibold bg-zinc-50 dark:bg-zinc-900">Points Table</div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.teamName} className="odd:bg-white even:bg-zinc-50/50 dark:odd:bg-zinc-950 dark:even:bg-zinc-900/30">
                <td className="p-2 whitespace-nowrap">{r.teamName}</td>
                <td className="p-2 text-right">{r.matchesPlayed}</td>
                <td className="p-2 text-right">{r.wins}</td>
                <td className="p-2 text-right">{r.losses}</td>
                <td className="p-2 text-right">{r.noResults}</td>
                <td className="p-2 text-right">{r.netRunRate.toFixed(3)}</td>
